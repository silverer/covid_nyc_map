...
server <- function(input, output) {
  ...
  output$census_plot <- renderPlotly({
    labs = get_pretty_labels()
    temp_inputs = choro_inputs %>% 
      rename(plot_names)
    p = build_choropleth(temp_inputs, rev_names[input$demo_var],
                         labs[1])
    ggplotly(p)
  })
  ...
  output$cov_plot <- renderPlotly({
    ...
    temp_inputs = choro_inputs %>% 
      rename(plot_names)
    p = build_choropleth(temp_inputs, rev_names[input$outcome_var],
                         labs[2])
    ggplotly(p)
  })
